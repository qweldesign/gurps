...
// 恐慌状態 (幻惑ではない場合): 前衛にいれば後列へ「移動」を, 後列にいれば「待機」を選択する
//   (幻惑状態を優先するのは, 幻惑中は「移動」自体が選択不可のため. 恐慌の定義自体は Action.ts の開幕時自動実行と同一)
...
// 幻惑・恐慌状態の場合はそちらを優先し, actor.tactic に対応するハンドラーが無い場合は「待機」を返す
