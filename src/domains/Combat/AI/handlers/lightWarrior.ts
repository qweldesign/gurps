 ...
 *
 * 1. 移動
 * 後衛にいれば前衛に移動 (優先順位 1.左翼, 2.右翼, 3.中央)
 ...
 * 自身の防御目標値が8以下なら全力攻撃, 10なら全力防御,
 * その間(9)なら 50% の確率分岐で 全力攻撃 か 全力防御 へ
 * それ以外(11以上)なら 6. へ
 *
 * 6. 準備/攻撃/牽制
 * 準備が必要な場合は準備
 ...
 * 優先順位 1.中央, 2.左翼, 3.右翼
 */
export const lightWarrior: TacticHandler = (actor, state) => {
  const { availability, target } = state.action!

  // 1. 移動
  if (actor.position === 'back') {
    const position = (['left', 'right', 'center'] as const).find(pos => availability.move[pos])
