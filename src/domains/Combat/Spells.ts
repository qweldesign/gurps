 ...
 * 発動判定成功で無条件に適用するが, 移行は即座には行わない (法術発動自体はターンを終えないため). 術者自身のターンが完全に
 * 終わった時点で, 対象のターンへその場で移行する (通常の行動順を待たない, その場限りの1ターンのみの制御. Health.puppeted は移行中のみ true になる)
 ...
 * shootPenalty が true の陣営に属するユニットは, 射撃武器の距離による修正 (distanceMod) が2倍になる (「守りの風」)
 ...
 * 術者自身の陣営が「守りの風」の影響下 (CombatState.shootPenalty[術者の陣営] が true) にある場合は2倍になる点も射撃武器と共通)
 * 対象が単一に定まる術 (spellType: 'shoot' 全般, および spellType: 'resist' で対象が敵 (術者と別陣営) の場合) は,
 * その対象の配置に応じて前列 -1 / 後列 -2 (「守りの風」の影響下ではそれぞれ -2 / -4) のペナルティを課す
 ...
 * 発動判定が1回のみで個々の対象の位置を一意に定められないため, dmg/flash 効果を持つものに限り, 位置によらず一律 -2 (「守りの風」の影響下では -4) とする
